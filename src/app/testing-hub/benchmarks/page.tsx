...
export default async function BenchmarksPage({ searchParams }: BenchmarksPageProps) {
  ...
  const benchmarks = filterTestingHubEntries(entries, {
    category: "benchmarks",
    status: sp.status,
    search: sp.search,
  });
      ...
      <section className="border-b border-white/10 bg-gradient-to-b from-white/5 to-transparent">
        <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
          ...
          <p className="text-lg text-white/70 max-w-3xl">
            Persistent benchmark entries for tracking agent performance trends, quality, and operational efficiency.
          </p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
        <div className="mb-6 flex items-center justify-between">
          <div className="text-white/70">
            Showing <strong className="text-white">{benchmarks.length}</strong> benchmark entries
          </div>
          <a href="/api/testing-hub?category=benchmarks" className="text-sm text-white/60 hover:text-white transition-colors">
            API endpoint →
          </a>
        </div>
          ...
          {benchmarks.map((entry) => (
            <article key={entry.id} className="rounded-xl border border-white/10 bg-white/5 p-6">
              <div className="mb-3 flex items-center justify-between gap-3">
                <span className="rounded-full border border-purple-500/40 bg-purple-500/10 px-3 py-1 text-xs uppercase tracking-wide text-purple-300">
                  {entry.status}
                </span>
        ...
        {benchmarks.length === 0 && (
          <div className="rounded-xl border border-white/10 bg-white/5 p-12 text-center text-white/60">
            No benchmark entries matched your filters.
          </div>
        )}
      </div>
    </div>
