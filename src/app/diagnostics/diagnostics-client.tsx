        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full h-32 px-4 py-3 bg-background border border-white/10 rounded-lg font-mono text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
            />
          </div>
          <button
            onClick={runDiagnostics}
            disabled={loading || !configInput.trim()}
            className="px-6 py-2.5 bg-primary text-background font-semibold rounded-lg hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
          >
            {loading ? (
              <>
